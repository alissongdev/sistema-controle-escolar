...
import { Atividade, AtividadeRealizada, Periodo, TipoAtividade } from "../../shared/interfaces/atividade";
...
@Component({
  ...
  template: `
    <div class="max-w-md mx-auto bg-white shadow-md border rounded-lg overflow-hidden p-5">
      <h3 class="text-xl font-semibold mb-3">Cadastrar Novas Atividades</h3>
      <form (ngSubmit)="onSubmit()">
        <div class="mb-4">
          <label class="block text-gray-700">Criança</label>
          <select [(ngModel)]="selectedCrianca" name="selectedCrianca" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
        ...
        <div class="mb-4">
          ...
          <div *ngFor="let atividade of atividadesConfig; let i = index" class="mb-2 p-2 border rounded">
            <div>
              <input type="checkbox" [(ngModel)]="atividade.selected" name="atividade-{{i}}" class="mr-2"/>
              {{ atividade.tipoAtividade }}
            </div>
            <div class="mt-2" *ngIf="atividade.selected">
              <label class="block text-gray-700">Status</label>
              <select [(ngModel)]="atividade.status" name="status-{{i}}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                <option *ngFor="let status of atividadeRealizadas" [ngValue]="status">
                  {{ status }}
                </option>
              </select>
            </div>
            <div class="mt-2" *ngIf="atividade.selected">
              ...
              <select [(ngModel)]="atividade.periodo" name="periodo-{{i}}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
        ...
        <button type="submit" class="w-full bg-blue-500 text-white p-2 rounded-md">Cadastrar Atividades</button>
      </form>
