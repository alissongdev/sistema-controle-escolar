...
export class RecadoFormComponent implements OnChanges {
  @Input() recadoToEdit: Recado | null = null;
  @Output() recado = new EventEmitter<Recado>();
  @Output() update = new EventEmitter<Recado>();
  ...
  nomeEmailRemetente = (() => {
    return this.isUserMonitor
      ? 'A Escola'
      : this.loginService.getResponsavel()!.nome +
          ' (' +
          this.loginService.getResponsavel()!.email +
          ')';
  })();
  ...
  ngOnChanges() {
    if (this.recadoToEdit) {
      this.recadoForm.patchValue(this.recadoToEdit);
    }
  }
